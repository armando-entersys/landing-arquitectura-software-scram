...
function openGoogleCalendar() {
  const now = new Date();
  // Agendar para el siguiente día hábil a las 10:00 AM
  const next = new Date(now);
  next.setDate(next.getDate() + 1);
  // Si cae sábado o domingo, mover al lunes
  ...
  const params = new URLSearchParams({
    action: 'TEMPLATE',
    text: 'Sesión de Arquitectura de Software - Scram Consulting',
    details:
    ...
    location: 'Google Meet (se enviará enlace)',
          ...
          <div className="flex flex-col md:flex-row items-center gap-5 md:gap-8">
            ...
            <div className="flex-shrink-0">
              <div className="relative w-24 h-24 md:w-40 md:h-40 rounded-full overflow-hidden bg-gradient-to-br from-scram-primary to-scram-secondary p-1">
                <div className="w-full h-full rounded-full overflow-hidden relative">
                  <Image
                    src="/images/team/armando.png"
                    alt={content.team.contact.name}
                    fill
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-2 md:gap-3 justify-center md:justify-start">
                <button
                  onClick={() => {
                    tracker.trackCTA('Agendar llamada', 'team', 'google-calendar');
                    openGoogleCalendar();
                  }}
                ...
                <button
                  onClick={() => {
                    tracker.trackCTA('WhatsApp directo', 'team', 'whatsapp');
                    const msg = encodeURIComponent(
                      'Hola Ing. Armando, me interesa conocer más sobre los servicios de arquitectura de software de Scram Consulting.'
                    );
                    window.open(`https://wa.me/522211065056?text=${msg}`, '_blank');
                  }}
                  className="px-5 py-2.5 md:px-6 md:py-3 bg-[#25D366] hover:bg-[#128C7E] text-white font-semibold text-sm md:text-base rounded-pill transition-all duration-300 hover:-translate-y-0.5 flex items-center justify-center gap-2"
                >
